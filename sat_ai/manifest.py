...
class InputSpec:
    ...
    def validate(self) -> None:
        ...
        if self.normalization_id != "legacy-dtype-range-v1" or self.normalization_kind != "dtype-range":
            raise ValueError("MVP normalization must be the pinned dtype-range contract")
...
class ModelManifest:
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "ModelManifest":
        ...
        result = cls(
            ...
            threshold_mapping_id=str(_require(output, "threshold_mapping_id", "manifest output")),
            threshold_lut_sha256=str(_require(output, "threshold_lut_sha256", "manifest output")),
    ...
    def validate(self) -> None:
        ...
        if self.assurance_level != "demo_non_validated":
            raise ValueError("MVP default assurance level is demo_non_validated")
        if self.threshold_mapping_id != "logit-bp-f32-lut-v1":
            raise ValueError("MVP threshold mapping is logit-bp-f32-lut-v1")
        if len(self.threshold_lut_sha256) != 64:
            raise ValueError("threshold_lut_sha256 must be a SHA-256 digest")
